...
function nowSec() { return Math.floor(Date.now() / 1000).toString(); }
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const pasteCmd = vscode.commands.registerCommand("pasteVector.pasteVector", async () => {
    ...
    const makeOutAbs = (ext: string) =>
      path.join(docDir, expandTemplate(tpl, { documentBaseName: docBase, unixTime, fileExtName: ext }));
  ...
  const showTypesCmd = vscode.commands.registerCommand("pasteVector.showClipboardTypes", async () => {
    ...
    for (const entry of await listClipboardTypes(preferBackend)) {
      if (entry.error) {
        ...
        out.appendLine(`${ts()} backend=${entry.kind} types=${entry.types.length}`);
        out.appendLine(entry.types.map((t) => `- ${t}`).join("\n"));
      }
    }
    if (isWSL()) {
      out.appendLine(`${ts()} backend=wsl note=Windows clipboard formats accessed via powershell.exe`);
    }
  });
